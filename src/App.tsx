...
const revokeReasons = [
  ...
  'фича вышла за согласованную аудиторию или платформу',
...
function getResultTitle(result: ResultType) {
  ...
  if (result === 'approval') {
    return 'Нужно согласование с SX Goods'
  }

  return 'Достаточно информировать SX Goods'
}
    ...
    <main className="page">
      <section className="hero">
        ...
        <div className="hero__content glowTarget">
          <h1>
            <span>Проверьте, как&nbsp;согласовать</span>
      ...
      <section className="section quiz glowTarget" id="quiz">
        <div className="section__heading">
          ...
          <h2>{keepShortWords('Интерактивный чек-лист согласования')}</h2>
          <p>
            {keepShortWords(
      ...
      <section className="section revokeSection glowTarget" id="revoke">
        <div className="section__heading">
          ...
          <p>
            {keepShortWords(
              'Это не запасной путь вместо согласования. Если чек-лист показывает согласование, команда приходит до запуска. Если изменение запустили без нужного согласования или после запуска появился риск для подачи, SX Goods может попросить остановить тест, откатить или отключить изменение.',
            )}
          </p>
        </div>
      ...
      <section className="section glowTarget" id="criteria">
        ...
        <div className="criteriaReference">
          {['Стоп-фактор', 'Согласование', 'Информирование'].map((title) => {
            const items =
              title === 'Стоп-фактор'
                ? [
                    {
